...
@Suite("App Tests")
struct TokyoDisneyResortAPITests {
    ...
    func tdlAttractionRoute() async throws {
        try await withDependencies {
            $0.attractionRepository = MockAttractionRepository(
                getIntegratedAttractionInfoHandler: { _, _ in
                    return []
                }
            )
        } operation: {
            ...
            try await withApp { app in
                ...
                try await app.testing().test(.GET, "tokyo_disney_resort/tdl/attraction", afterResponse: { res1 async throws in
                    ...
                    try await app.testing().test(.GET, "tokyo_disney_resort/tdl/attraction", afterResponse: { res2 async throws in
                        ...
                        let attractions1 = try? JSONDecoder().decode([Attraction].self, from: Data(buffer: res1.body))
                        let attractions2 = try? JSONDecoder().decode([Attraction].self, from: Data(buffer: res2.body))
                        ...
                        if let attr1 = attractions1, let attr2 = attractions2, attr1.count == attr2.count {
                            for i in 0..<attr1.count {
                                #expect(attr1[i].attractionName == attr2[i].attractionName)
        ...
        try await withDependencies {
                ...
                getIntegratedAttractionInfoHandler: { _, _ in
                    return [
                        .init(
                            ...
                            operatingStatus: .init(
                                facilityID: "0001",
                                facilityName: "テストアトラクション",
                                standbyTime: StandbyTime.string("30"),
                                operatingStatus: "案内中",
                                dpaStatus: "販売なし",
                                fsStatus: "販売なし",
                                updateTime: "9:00",
            ...
            try await withApp { app in
                try await app.testing().test(.GET, "tokyo_disney_resort/tdl/attraction", afterResponse: { res async in
                    ...
                    // JSONレスポンスを解析
                    let attractions = try? JSONDecoder().decode([Attraction].self, from: Data(buffer: res.body))
                    
                    #expect(attractions?.count == 1)
                    #expect(attractions?[0].attractionName == "テストアトラクション")
